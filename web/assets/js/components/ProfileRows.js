...
import { readFileAsDataUrl } from '../modules/utils.js';
...
export function createSkillRow(item = { category: "", items: [] }) {
    ...
    skills = skills.map(s => typeof s === 'string' ? s.replace(/[ \s×]+$/, '').trim() : s);
    ...
    const renderPills = () => {
        ...
        skills.forEach((skill, idx) => {
            ...
            remove.textContent = '×';
            remove.onclick = () => { skills.splice(idx, 1); renderPills(); };
    ...
    input.onkeydown = (e) => {
        if (e.key === 'Enter') {
            ...
            if (val && !skills.includes(val)) { skills.push(val); input.value = ''; renderPills(); }
        }
    };
    renderPills();
    updateUIStrings();
