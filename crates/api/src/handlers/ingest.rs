...
use domain::{Offre, OffreId, Slug, OffreStructured};
use ports::Scraper;
use uuid::Uuid;
use chrono::Utc;
...
pub async fn ingest_handler(
    ...
    let lines: Vec<&str> = payload.input.lines()
        .map(|l| l.trim())
        .filter(|l| !l.is_empty())
        .collect();
    ...
    for item in lines {
        let (intitule, entreprise, content, url) = if item.starts_with("http") {
            ...
            match scraper.scrape(item).await {
                Ok(res) => {
                    (format!("Job from {}", item), "Scrapped Corp".to_string(), res.raw_text, item.to_string())
                }
                Err(e) => {
                    return Err(ApiError::BadRequest(format!("Erreur de scraping pour {}: {}", item, e)));
                }
            }
        } else {
            // Texte brut
            ("Manual Job Entry".to_string(), "Unknown".to_string(), item.to_string(), "manual".to_string())
        };
        ...
        state.offre_repo.upsert(&offre).await
        ...
        state.instance_repo.upsert(&instance).await
            .map_err(|e| ApiError::Internal(e.to_string()))?;

        results.push(slug.to_string());
    }

    Ok(Json(serde_json::json!({ "status": "ok", "ingested": results })))
}
