...
impl IndexProfilUseCase {
    pub fn new(
        ...
        Self { profils, chunks, embedder }
    }

    pub async fn execute(&self, profil_id: ProfilId) -> Result<(), AppError> {
        let profil = self.profils.get_by_id(profil_id).await?
        ...
        if let Some(pitch) = profil.content.get("pitch").and_then(|v| v.as_str()) {
            chunks_to_index.push(("Pitch personnel".to_string(), pitch.to_string(), ChunkKind::Experience));
        }

        // 3. Parser les expériences
        if let Some(exps) = profil.content.get("experiences").and_then(|v| v.as_array()) {
            for exp in exps {
                let role = exp.get("role").and_then(|v| v.as_str()).unwrap_or("Inconnu");
                let company = exp.get("company").and_then(|v| v.as_str()).unwrap_or("Inconnue");
                let desc = exp.get("description").and_then(|v| v.as_array())
        ...
        if let Some(projs) = profil.content.get("projects").and_then(|v| v.as_array()) {
            for proj in projs {
                let title = proj.get("role").and_then(|v| v.as_str()).unwrap_or("Projet");
                let name = proj.get("company").and_then(|v| v.as_str()).unwrap_or("");
                let desc = proj.get("description").and_then(|v| v.as_array())
                    .map(|a| a.iter().map(|v| v.as_str().unwrap_or("")).collect::<Vec<_>>().join("\n"))
                    .unwrap_or_default();
        ...
        println!("✨ Génération des embeddings pour {} chunks...", chunks_to_index.len());

        for (titre, content, kind) in chunks_to_index {
            if content.is_empty() { continue; }

            let embeddings = self.embedder.embed(&[&content], EmbedMode::Document).await
