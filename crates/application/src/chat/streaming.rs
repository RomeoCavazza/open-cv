...
use std::sync::Arc;
use ports::{MessageRepo, InstanceRepo, ProfilRepo, BoxStream, LlmError};
            ...
            |(mut stream, mut instance, mut full_response, message_repo, instance_repo)| async move {
                match stream.next().await {
                    Some(Ok(token)) => {
                        ...
                        Some((Ok(token), (stream, instance, full_response, message_repo, instance_repo)))
                    }
                    Some(Err(e)) => Some((Err(e), (stream, instance, full_response, message_repo, instance_repo))),
                    None => {
