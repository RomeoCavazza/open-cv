Romeo CAVAZZA
Alternance — Ingénieur Logiciel & IA

Étudiant en Master Intelligence artificielle, je souhaite contribuer aux projets logiciels et data de Safran en combinant développement Python/TypeScript, automatisation et pratiques d'ingénierie robustes.

Durée : 24 mois — à partir de septembre 2026
Rythme : 6 semaines en entreprise / 2 semaines en cours

---

CONTACT

Paris, 11e
06.99.25.67.58
romeo.cavazza@gmail.com
www.romeocavazza.com
in/romeo-cavazza/
github.com/RomeoCavazza

---

EXPERIENCES

Tco-core — Pipeline de Données Haute Fréquence (2025 – présent)
• Système d'acquisition 100 Hz : firmware Arduino C++ et gateway Python
• Traitement de signal, stockage InfluxDB et monitoring Grafana

ShellGeist — Agent IA Terminal (2025 – présent)
• Agent IA local en Python pour exécuter et superviser des tâches de développement
• Plugin Neovim en Lua, IPC socket, LLM et environnement Nix

Setup-os — Infrastructure NixOS (2025 – présent)
• Système Linux déclaratif avec Nix Flakes, Docker et automatisation d'environnements
• Stack d'observabilité avec Grafana, Prometheus, Loki et secrets via SOPS

AI Automation Engineer – R&D Intern
Revolvr, Paris, France
Stage de 6 mois (Février-Juillet 2025)
• Automatisation de la veille et analyse de tendances avec IA
• Intégration d'APIs sociales (TikTok, Meta) dans des workflows métier
• Déploiement de workflows LLM pour monitoring et détection de signaux

---

FORMATIONS

EPITECH, Paris, France (2026 – 2028)
Master of Science – Intelligence artificielle
EPITECH, Paris, France (2025 – 2026)
Pré-MSc — Python, APIs, SQL, Docker et développement fullstack
HarvardX, edX
CS50W — Introduction to Computer Science
CS50W — Web Programming with Python and JavaScript

---

COMPETENCES

Software
Python, TypeScript, C++, SQL, Bash
IA & automatisation
LangChain, OpenAI API, Ollama, scikit-learn, Qdrant
Backend & data
FastAPI, PostgreSQL, Redis, Alembic, Grafana
Infra & DevOps
Linux, Docker, Git, GitHub Actions, NixOS
Embarqué & systèmes
C, C++, Arduino, UART, InfluxDB

---

LANGUES

Français : natif
Anglais : C1
Allemand : B1