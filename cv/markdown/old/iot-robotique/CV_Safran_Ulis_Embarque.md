Romeo CAVAZZA
Alternance — Ingénieur Logiciel Embarqué

Étudiant en Master Robotique & IoT, je souhaite contribuer à des développements embarqués et Linux bas niveau pour des produits de télémesure et d'acquisition de données.

Durée : 24 mois — à partir de septembre 2026
Rythme : 6 semaines en entreprise / 2 semaines en cours

---

CONTACT

Paris, 11e
06.99.25.67.58
romeo.cavazza@gmail.com
www.romeocavazza.com
in/romeo-cavazza/
github.com/RomeoCavazza

---

EXPERIENCES

Tco-core — Pipeline de Données Haute Fréquence (2025 – présent)
• Système d'acquisition 100 Hz : firmware Arduino C++ et gateway Python
• Traitement de signal, stockage InfluxDB et monitoring Grafana

Setup-os — Infrastructure NixOS (2025 – présent)
• Système Linux déclaratif avec Nix Flakes, Docker et automatisation d'environnements
• Stack d'observabilité avec Grafana, Prometheus, Loki et secrets via SOPS

AI Automation Engineer – R&D Intern
Revolvr, Paris, France
Stage de 6 mois (Février-Juillet 2025)
• Automatisation de la veille et analyse de tendances avec IA
• Coordination technique autour d'APIs sociales (TikTok, Meta)
• Déploiement de workflows LLM pour livrables de veille et signaux stratégiques

AI Consultant & Formateur
FCNet, Paris, France
Stage de 6 mois (Janvier-Juin 2026)
• Conception de modules de formation IA générative et stratégies d'adoption
• Animation de cours en ligne et vulgarisation de cas d'usage métiers
• Production de supports de formation techniques et documentaires

---

FORMATIONS

EPITECH, Paris, France (2026 – 2028)
Master of Science – Robotique & IoT
EPITECH, Paris, France (2025 – 2026)
Pré-MSc — C, Python, Linux, algorithmique et systèmes
HarvardX, edX
CS50W — Introduction to Computer Science
CS50W — Web Programming with Python and JavaScript

---

COMPETENCES

Langages systèmes
C, C++, Python, Bash, SQL
Linux bas niveau
Linux, Git, GDB, Valgrind, Systemd
Drivers & outillage
CMake, Docker, GitHub Actions, Bash, Neovim
Data & acquisition
InfluxDB, Grafana, pyserial, Arduino, UART
Intégration logicielle
FastAPI, REST, PostgreSQL, Redis, Docker

---

LANGUES

Français : natif
Anglais : C1
Allemand : B1